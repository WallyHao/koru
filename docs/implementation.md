...
- A provider boundary (`docs/providers.md`): a versioned strict `config.toml` with atomic writes and a scoped writer lock, environment credentials with redaction, three service identities, a bounded catalog model with a fixture source, and adapter capability preflight that stops unsupported tool or schema requests before dispatch. `koru model`, `koru model <service>[/<model>]`, and `koru variant` select and persist; `koru model update` reports `unsupported_capability` until a transport exists.
...
- No transport is implemented, so `koru <command>` still returns a nonzero unsupported-capability error, `koru model update` cannot fetch live metadata, and the bridge is exercised through the library and the fake service. There is no terminal approval prompt, OS effect adapter, durable catalog cache, or Git service.
- The JSON Schema subset is enforced and adapter capabilities are checked, but the declared per-service variants are provisional until real adapters and catalogs provide them.
...
- Release binary: 1,634,840 bytes (stripped, thin LTO, Rust 1.98.1). Measurement only, not a target.
...
- 127 tests cover suspend/resume, serial tool ordering, several AI calls per workflow, nested-call rejection, turn limit, tool/model budget exhaustion, cancellation while waiting with a bounded latency, an uncooperative service that cannot block shutdown, provider failure, JSON round-tripping, schema compile/validate matrices, capability preflight, config parsing/atomicity/concurrent writers, credential redaction, and catalog parsing/refresh.

## Next implementation gates

1. Pin an HTTP client, implement one provider protocol end to end, and wire the live catalog through the `CatalogSource` boundary with durable caching.
