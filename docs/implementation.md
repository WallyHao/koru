...
- A terminal adapter owns prompts, approval, and output routing. It emits restrained ANSI color only when the target stream is an interactive terminal and `NO_COLOR` is unset and `TERM` is not `dumb`; redirected output stays clean. The task prompt runs a raw-mode line editor that is UTF-8- and display-width-aware, so backspace removes a whole CJK or emoji cell instead of leaving half of a double-width glyph behind, and approval accepts `y`/`yes` (anything else denies).
